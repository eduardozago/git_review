...
export function LandingHowItWorks() {
  const { how } = useCopy(landingCopy)

  return (
    <section id="how" className="border-t border-border px-6 py-24 md:px-8">
      <div className="mx-auto max-w-275">
        <div className="mb-14">
          <p className="mono m-0 text-xs tracking-wide text-primary uppercase">{how.eyebrow}</p>
        ...
        <div className="grid gap-6 md:grid-cols-3">
          {how.steps.map((step) => (
            <div key={step.n}>
              <div className="mono text-xs tracking-wide text-dim">STEP / {step.n}</div>
              <h3 className="mt-4 text-5.5 font-semibold tracking-tight text-strong">{step.title}</h3>
