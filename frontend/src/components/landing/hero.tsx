...
export function LandingHero({ report }: LandingHeroProps) {
  const { hero, nav } = useCopy(landingCopy)

  return (
    <section className="relative overflow-hidden px-6 pb-20 pt-16 md:px-8 md:pb-24 md:pt-28">
      <div aria-hidden="true" className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_800px_400px_at_50%_0%,var(--accent-soft)_0%,transparent_60%)]" />
      <div aria-hidden="true" className="pointer-events-none absolute inset-0 bg-[linear-gradient(var(--grid)_1px,transparent_1px),linear-gradient(90deg,var(--grid)_1px,transparent_1px)] bg-size-12 mask-[radial-gradient(ellipse_60%_50%_at_50%_30%,black_0%,transparent_75%)]" />

      <div className="relative mx-auto max-w-275 text-center">
        <Badge tone="outline" size="lg" className="mb-7">
          <Sparkles size={12} />
          <span className="mono text-2.75 tracking-wide uppercase">{hero.eyebrow}</span>
        </Badge>
