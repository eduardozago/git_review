...
export function HeroPreviewCard({ report }: HeroPreviewCardProps) {
  const { report: reportCopy } = useCopy(landingCopy)
    ...
    <div className="overflow-hidden rounded-2xl border border-border bg-card shadow-pop">
      <div className="flex items-center gap-2.5 border-b border-border bg-elev px-4 py-3">
        <div className="flex gap-1.5">
          {[0, 1, 2].map((i) => (<span key={i} className="size-2.5 rounded-full bg-faint" />))}
        </div>
      ...
      <div className="grid items-center gap-8 p-7 md:grid-cols-[1.1fr_1fr]">
        <div>
          <div className="mb-3 flex items-center gap-2">
            ...
            <span className="mono text-3.25 text-muted-foreground">{repoPath}</span>
          </div>
          <div className="flex items-baseline gap-3">
            <span className="text-6xl sm:text-7xl lg:text-9xl font-semibold leading-none tracking-tighter text-strong">{report.score}</span>
            <span className="mono text-sm text-dim">/ 100</span>
          </div>
          <div className="mt-3.5 flex flex-wrap items-center gap-2">
            <Badge tone="accent" icon={Check} size="lg">{reportCopy.level}</Badge>
            <Badge tone="outline" size="lg"><span className="mono text-2.75">{reportCopy.delta}</span></Badge>
          </div>
        </div>
          ...
          {report.dimensions.map((dim) => (
            <div key={dim.id}>
              <div className="mb-1.5 flex items-center justify-between">
                ...
                <span className="mono text-xs text-muted-foreground">{dim.score}</span>
              </div>
