...
export function LandingReportPreviewSection({ report }: LandingReportPreviewSectionProps) {
  ...
  const activeDimension = report.dimensions.find((d) => d.id === tab) ?? report.dimensions[0]
      ...
      <div className="mx-auto max-w-275">
        ...
        <div className="overflow-hidden rounded-2xl border border-border bg-card">
          <div className="flex flex-wrap items-center justify-between gap-4 border-b border-border px-5 py-4">
            ...
            <Badge tone="accent" icon={Check}>{preview.liveDemo}</Badge>
          </div>
            ...
            {tabs.map((item) => (
              <button
                ...
                className={[
                  ...
                  tab === item.id ? "border-primary text-strong" : "border-transparent text-muted-foreground hover:text-foreground",
                ].join(" ")}
              >
                {item.label}
              </button>
          ...
          <div className="grid gap-6 p-6 md:grid-cols-2">
            {tab === "overview" ? (
              <>
                <div>
                  <div className="mono text-xs tracking-wide text-dim uppercase">{preview.summary}</div>
                  <p className="mt-3 text-pretty text-3.75 leading-relaxed text-foreground">{preview.summaryBlurb}</p>
                  <div className="mt-5 flex flex-col gap-3">
                    {report.dimensions.map((dim) => (
                      <div key={dim.id} className="flex items-center gap-3">
                        <span className="w-32.5 shrink-0 text-3.25 text-muted-foreground">{dim.label}</span>
                        <div className="min-w-0 flex-1"><ProgressBar value={dim.score} animated /></div>
                        <span className="mono w-7 shrink-0 text-right text-xs text-foreground">{dim.score}</span>
                      </div>
                    ))}
                  </div>
                </div>
                <CodeBlock code={report.dimensions[0].snippet.code} file={report.dimensions[0].snippet.file} startLine={report.dimensions[0].snippet.startLine} comment={report.dimensions[0].snippet.comment} />
              </>
            ) : (
              <>
                <div>
                  <p className="mt-0 text-sm leading-relaxed text-muted-foreground">{activeDimension.tagline}</p>
                  <div className="mt-4 flex flex-col gap-3">
                    {activeDimension.positives.map((item) => (
                      <div key={item.h} className="flex items-start gap-2.5">
                        <Check size={14} className="mt-0.5 shrink-0 text-primary" />
                        <span className="text-sm text-foreground">{item.h}</span>
                      </div>
                    ))}
                ...
                <CodeBlock code={activeDimension.snippet.code} file={activeDimension.snippet.file} startLine={activeDimension.snippet.startLine} comment={activeDimension.snippet.comment} />
              </>
            )}
          </div>
        </div>
